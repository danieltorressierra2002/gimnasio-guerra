...
export default function UserDashboard() {
  ...
  const estado = calcularEstadoMembresia(datosUsuario.fecha_vencimiento)
  const config = ESTADO_CONFIG[estado]
  const dias = calcularDiasRestantes(datosUsuario.fecha_vencimiento)
  ...
  return (
    <div className="min-h-screen bg-carbon texture-floor">
      ...
      <main className="max-w-md mx-auto px-4 py-8">
        {seccion === "membresia" && (
          <>
            <div className={`bg-carbon-surface border-2 rounded-2xl p-6 text-center shadow-plate ${config.glow}`}
              style={{ borderColor: estado === "vencido" ? "#DC2626" : estado === "porVencer" ? "#D97706" : "#22C55E" }}>
              <div className="flex justify-center mb-4">
                {datosUsuario.foto_url ? (
                  <img src={datosUsuario.foto_url} alt={datosUsuario.nombre} className="w-24 h-24 rounded-full object-cover border-2 border-steel" />
                ) : (
              ...
              <p className="text-bone-dim text-sm mt-4">
                {estado === "vencido"
                  ? `Tu membresía venció el ${formatearFecha(datosUsuario.fecha_vencimiento)}.`
                  : `Vence el ${formatearFecha(datosUsuario.fecha_vencimiento)} (${dias} día${dias === 1 ? "" : "s"} restante${dias === 1 ? "" : "s"}).`}
              </p>
            </div>

            <div className="mt-5 bg-carbon-surface border border-steel/40 rounded-xl divide-y divide-steel/30">
              <Detalle label="Método de pago" valor={datosUsuario.metodo_pago === "online" ? "Pago en línea" : "Pago directo"} />
              <Detalle label="Entrenador personal" valor={datosUsuario.tiene_entrenador ? "Sí" : "No"} />
              <Detalle label="Inicio del periodo" valor={formatearFecha(datosUsuario.fecha_inicio_pago)} />
            ...
            {esActivo && ofertasFlash.some(o => o.visibilidad === "miembros_activos" && o.activa) && (
              <button onClick={() => setSeccion("ofertas")}
