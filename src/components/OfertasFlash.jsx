...
const VACIO = {
  ...
  visibilidad: "todos", activa: true, fecha_fin: "",
}
...
export default function OfertasFlash({ esAdmin }) {
  ...
  async function cargarOfertas() {
    const { data } = await supabase.from("ofertas_flash").select("*").order("created_at", { ascending: false })
    setOfertas(data || [])
    setCargando(false)
  }

  async function toggleActiva(oferta) {
    await supabase.from("ofertas_flash").update({ activa: !oferta.activa }).eq("id", oferta.id)
    cargarOfertas()
  }

  async function guardarOferta(datos) {
    if (ofertaSeleccionada) {
      await supabase.from("ofertas_flash").update(datos).eq("id", ofertaSeleccionada.id)
    } else {
      await supabase.from("ofertas_flash").insert(datos)
    }
  ...
  async function eliminarOferta(oferta) {
    if (!confirm(`¿Eliminar "${oferta.titulo}"?`)) return
    await supabase.from("ofertas_flash").delete().eq("id", oferta.id)
    setModalAbierto(false)
    cargarOfertas()
  }
    ...
    <div className="space-y-4">
      {ofertas.length === 0 ? (
        <div className="text-center py-14 border border-dashed border-steel/40 rounded-xl">
          ...
          {esAdmin && (
            <button onClick={() => { setOfertaSeleccionada(null); setModalAbierto(true) }} className="mt-4 text-forge-glow font-medium underline">Crear la primera</button>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          {ofertas.map(oferta => (
            <div key={oferta.id} className={`bg-carbon-surface border rounded-xl p-4 flex items-start gap-4 ${oferta.activa ? "border-blood/40" : "border-steel/40 opacity-60"}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  {oferta.fecha_fin && <span className="text-xs text-bone-dim">Hasta: {oferta.fecha_fin}</span>}
                </div>
                {esAdmin && (
      ...
      {esAdmin && (
        <button onClick={() => { setOfertaSeleccionada(null); setModalAbierto(true) }}
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-carbon-surface border border-steel/40 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[92vh] overflow-y-auto shadow-plate">
        <div className="sticky top-0 bg-carbon-surface border-b border-steel/30 px-5 py-4 flex items-center justify-between">
          <h2 className="font-display text-lg text-bone uppercase tracking-wide">{esEdicion ? "Editar oferta" : "Nueva oferta flash"}</h2>
        ...
        <form onSubmit={manejarGuardar} className="p-5 space-y-4">
          ...
          <Campo label="Válida hasta (opcional)">
            <input type="date" value={form.fecha_fin} onChange={(e) => actualizar("fecha_fin", e.target.value)} className="campo-input" />
          </Campo>
          <Campo label="Visibilidad">
            <div className="grid grid-cols-1 gap-2">
              <BotonToggle activo={form.visibilidad === "todos"} onClick={() => actualizar("visibilidad", "todos")}>🌍 Para todos</BotonToggle>
              <BotonToggle activo={form.visibilidad === "miembros_activos"} onClick={() => actualizar("visibilidad", "miembros_activos")}>⭐ Solo miembros activos</BotonToggle>
            </div>
          </Campo>
          <Campo label="Estado">
          ...
          <div className="flex gap-3 pt-2">
            {esEdicion && (
              <button type="button" onClick={() => onDelete(ofertaExistente)} className="flex-1 bg-transparent border border-blood/40 text-blood-glow hover:bg-blood/10 font-medium py-3 rounded-lg transition-colors">Eliminar</button>
            )}
